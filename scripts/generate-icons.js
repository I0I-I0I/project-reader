...
try {
    ...
    execSync(
        `convert -background none "${tempLightPath}" -resize 16x16 "${path.join(STATIC_DIR, "favicon-16x16.png")}"`,
    )

    console.log("Generating favicon-32x32.png...")
    execSync(
        `convert -background none "${tempLightPath}" -resize 32x32 "${path.join(STATIC_DIR, "favicon-32x32.png")}"`,
    )

    console.log("Generating favicon.ico (multi-resolution)...")
    execSync(
        `convert -background none "${tempLightPath}" -define icon:auto-resize=16,32,48 "${path.join(STATIC_DIR, "favicon.ico")}"`,
    )

    console.log("Generating apple-touch-icon.png (180x180, dark background)...")
    execSync(
        `convert -background none "${tempDarkPath}" -resize 130x130 -gravity center -background "#1b1715" -extent 180x180 "${path.join(STATIC_DIR, "apple-touch-icon.png")}"`,
    )
    ...
    execSync(
        `convert -background none "${tempDarkPath}" -resize 86x86 -gravity center -background "#1b1715" -extent 120x120 "${path.join(STATIC_DIR, "apple-touch-icon-120x120.png")}"`,
    )
    ...
    execSync(
        `convert -background none "${tempDarkPath}" -resize 140x140 -gravity center -background "#1b1715" -extent 192x192 "${path.join(STATIC_DIR, "android-chrome-192x192.png")}"`,
    )

    console.log("Generating android-chrome-512x512.png (512x512, dark background)...")
    execSync(
        `convert -background none "${tempDarkPath}" -resize 380x380 -gravity center -background "#1b1715" -extent 512x512 "${path.join(STATIC_DIR, "android-chrome-512x512.png")}"`,
    )
